...
const SearchPage = () => {
    ...
    debouncedRef.current = debounce(async (query) => {
      try {
        ...
        try {
          ...
          console.log('[search] calling:', `${api.defaults?.baseURL || ''}/api/users?search=${query}`);
        } catch (e) {
          // ignore
        }

        const [usersResponse, postsResponse, groupsResponse, journalsResponse] = await Promise.all([
        ...
        console.error('Lỗi khi tìm kiếm (primary):', error?.response?.data || error.message || error);
        // Try public fallback endpoint for any error (useful while debugging routes/auth)
        try {
          console.log('[search] primary request failed, trying public fallback /api/users/public');
          const publicResp = await api.get('/api/users/public', { params: { search: query, limit: 20 } });
          setResults(prev => ({ ...prev, people: publicResp.data.data || [] }));
          setError(null);
          return;
        } catch (pubErr) {
          console.error('Lỗi khi gọi public fallback:', pubErr?.response?.data || pubErr.message || pubErr);
          const serverMessage = pubErr?.response?.data?.message || pubErr?.response?.data || pubErr.message;
          setError(serverMessage || (error?.response?.data?.message || error?.response?.data || error.message) || 'Lỗi khi tìm kiếm. Vui lòng thử lại.');
          return;
        }
      } finally {
        setLoading(false);
  ...
  useEffect(() => {
    return () => {
      if (debouncedRef.current && debouncedRef.current.cancel) debouncedRef.current.cancel();
    };
  }, []);
              ...
              return (
                ...
                >
                  <div className="d-flex align-items-center mb-2">
                    <img
                      src={item.userCreateID?.profile?.avatar || '/assets/images/user/1.jpg'}
                      className="rounded-circle"
                      alt=""
                      width="32"
                      height="32"
                ...
                >
                  <div className="d-flex align-items-center">
                    ...
                    <div className="ms-3">
                      ...
                      <small className="text-muted">{item.memberCount} thành viên</small>
                    </div>
                  </div>
                </div>
              );
            ...
            <div className="search-sidebar">
              <div className="list-group">
                ...
                >
                  ...
                  Mọi người
                  {results.people.length > 0 && <span className="badge rounded-pill bg-light text-dark float-end">{results.people.length}</span>}
                </button>
                <button
                ...
                >
                  ...
                  Bài viết
                  {results.posts.length > 0 && <span className="badge rounded-pill bg-light text-dark float-end">{results.posts.length}</span>}
                </button>
                <button
                ...
                >
                  ...
                  Nhóm
                  {results.groups.length > 0 && <span className="badge rounded-pill bg-light text-dark float-end">{results.groups.length}</span>}
                </button>
                <button
                  className={`list-group-item list-group-item-action ${activeTab === 'journals' ? 'active' : ''}`}
                  onClick={() => setActiveTab('journals')}
                >
                  <i className="ri-book-line me-2"></i>
                  Nhật ký
                  {results.journals.length > 0 && <span className="badge rounded-pill bg-light text-dark float-end">{results.journals.length}</span>}
                </button>
              </div>
            </div>
          </div>
