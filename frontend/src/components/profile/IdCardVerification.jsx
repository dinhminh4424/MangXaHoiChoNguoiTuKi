...
//   /* --------------------------------------------------------------- */
//   // Detect + lưu (SỬA: XỬ LÝ KHI KHÔNG CÓ KHUÔN MẶT)
//   const detectFaceInImage = async (file, setData, canvasRef) => {
//     const img = new Image();
//     const url = URL.createObjectURL(file);
//     img.src = url;
...
//           if (!detection) {
//             console.warn("Không phát hiện khuôn mặt trong ảnh");
//             setData({ detection: null, dataURL: null });
...
//               ctx.font = "20px Arial";
//               ctx.fillStyle = "red";
//               ctx.fillText("Không tìm thấy khuôn mặt", 10, 30);
//             }
//             resolve(false);
//             return;
//           }
...
//     if (!blob) {
//       alert("Không tạo được ảnh selfie!");
//       return;
//     }
...
//     img.src = tmpUrl;
//     img.onload = () => {
//       drawFaceOnCanvas(img, null, selfieCanvasRef.current);
//       URL.revokeObjectURL(tmpUrl);
//     };

//     const success = await processSelfie(blob);
//     if (!success) {
//       stopCamera(); // Cho phép bật lại camera
//     }
//   };

//   // SỬA: XỬ LÝ LỖI KHUÔN MẶT
//   const processSelfie = async (blob) => {
//     setLoading(true);
//     setSimilarity(null);
//     try {
//       const ok = await detectFaceInImage(blob, setSelfieData, selfieCanvasRef);
//       if (!ok) {
//         alert(
//           "Không phát hiện khuôn mặt trong selfie! Vui lòng chụp lại rõ mặt."
//         );
//         setSelfieBlob(null);
//         return false;
//       }
...
//       if (!cccdData.detection) {
//         alert("Lỗi: Không có khuôn mặt CCCD để so sánh!");
//         return false;
//       }
...
//     } catch (err) {
//       console.error(err);
//       alert("Lỗi xử lý selfie! Vui lòng thử lại.");
//       setSelfieBlob(null);
//       return false;
//     } finally {
//       setLoading(false);
...
//                 onClick={startCamera}
//                 className="btn btn-outline-primary"
//                 disabled={loading || !!selfieBlob || isCameraOn}
//               >
//                 Bật camera
//               </button>
//               <button
//                 onClick={captureSelfie}
//                 className="btn btn-success"
//                 disabled={loading || !isCameraOn || !!selfieBlob}
//               >
//                 {loading ? "Đang chụp..." : "Chụp ảnh"}
//               </button>
//             </div>
...
//                   <h6 className="fw-bold">Selfie đã chụp</h6>
//                   <canvas
//                     ref={selfieCanvasRef}
//                     className="w-100 border rounded shadow-sm"
//                     style={{ maxHeight: "250px" }}
...
//                 {/* CẢNH BÁO KHÔNG CÓ KHUÔN MẶT */}
//                 {selfieBlob && !loading && !selfieData.detection && (
//                   <div className="mt-3 p-3 bg-warning text-dark rounded text-center">
//                     <p className="mb-0">
//                       Không phát hiện khuôn mặt. Vui lòng chụp lại rõ mặt, không
//                       đeo kính, ánh sáng tốt.
//                     </p>
//                   </div>
//                 )}
...
//                     <div className="spinner-border text-primary"></div>
//                     <p className="mt-2">
//                       Đang nhận diện khuôn mặt và so sánh...
//                     </p>
//                   </div>
//                 )}

//                 {!loading && similarity !== null && selfieData.detection && (
...
//                         onClick={() => setStep(3)}
//                         className="btn btn-success px-4"
//                         disabled={similarity < SIMILARITY_THRESHOLD}
//                       >
//                         Tiếp tục
//                       </button>
//                     </div>
...
//                 onClick={confirmStep3}
//                 className="btn btn-success px-4"
//                 disabled={loading || similarity < SIMILARITY_THRESHOLD}
//               >
//                 {loading ? "Đang lưu..." : "Xác nhận & Lưu"}
//               </button>
//             </div>
