...
const Follow = require("../models/Follow");
const NotificationService = require("../services/notificationService");
const { logUserActivity } = require("../logging/userActivityLogger"); // GHI LOG HOẠT ĐỘNG
...
class FriendController {
  ...
  async sendFriendRequest(req, res) {
    try {
      ...
      logUserActivity?.({
        ...
        userId: requesterId,
        role: req.user.role,
        target: { type: "user", id: recipientId },
  ...
  async acceptFriendRequest(req, res) {
    try {
      ...
      const acceptor = await User.findById(currentUserId).select(
        "username fullName profile.avatar"
      );
      const requesterUser = await User.findById(friendRequest.requester).select(
        "username fullName profile.avatar"
      ...
      await NotificationService.createAndEmitNotification({
        ...
        title: "Yêu cầu kết bạn được chấp nhận",
        message: `${
          acceptor.fullName || acceptor.username
        } đã chấp nhận yêu cầu kết bạn của bạn`,
        data: { friendRequestId: friendRequest._id, friendId: friendship._id },
        priority: "medium",
      ...
      if (oldNotification) {
        oldNotification.type = "FRIEND_REQUEST_ACCEPTED";
        oldNotification.title = "Hai bạn đã trở thành bạn bè";
        oldNotification.message = `Bạn và ${
          requesterUser.fullName || requesterUser.username
        } đã trở thành bạn bè`;
        oldNotification.read = true;
        oldNotification.readAt = new Date();
        await oldNotification.save();
      ...
      logUserActivity?.({
        ...
        role: req.user.role,
        target: { type: "user", id: friendRequest.requester.toString() },
        description: "Chấp nhận yêu cầu kết bạn",
        payload: {
          requesterId: friendRequest.requester.toString(),
          friendRequestId: friendRequest._id.toString(),
  ...
  async rejectFriendRequest(req, res) {
    try {
      ...
      const rejector = await User.findById(currentUserId).select(
        "username fullName profile.avatar"
      );
      const requesterUser = await User.findById(friendRequest.requester).select(
        "username fullName profile.avatar"
      ...
      await NotificationService.createAndEmitNotification({
        ...
        title: "Yêu cầu kết bạn bị từ chối",
        message: `${
          rejector.fullName || rejector.username
        } đã từ chối yêu cầu kết bạn của bạn`,
      ...
      if (oldNotification) {
        oldNotification.type = "FRIEND_REQUEST_REJECTED";
        oldNotification.title = "Bạn đã từ chối lời mời kết bạn";
        oldNotification.message = `Bạn đã từ chối lời mời kết bạn từ ${
          requesterUser.fullName || requesterUser.username
        }`;
        oldNotification.read = true;
        oldNotification.readAt = new Date();
  ...
  // [POST] /api/friends/cancel/:requestId - Hủy yêu cầu
  async cancelFriendRequest(req, res) {
    try {
      const currentUserId = req.user.userId;
      const { requestId } = req.params;
      ...
      await friendRequest.populate([
        { Gathering: "requester", select: "username fullName profile.avatar" },
        { path: "recipient", select: "username fullName profile.avatar" },
      ]);
      ...
      logUserActivity?.({
        ...
        userId: currentUserId,
        role: req.user.role,
        target: { type: "user", id: friendRequest.recipient.toString() },
        description: "Hủy yêu cầu kết bạn",
        payload: {
          recipientId: friendRequest.recipient.toString(),
          friendRequestId: friendRequest._id.toString(),
        },
      });
  ...
  // [GET] /api/friends/requests - Danh sách yêu cầu
  async getFriendRequests(req, res) {
    try {
      const currentUserId = req.user.userId;
      ...
      let query = {};
      if (type === "received") {
        query = { recipient: currentUserId, status: "pending" };
      } else if (type === "sent") {
        query = { requester: currentUserId, status: "pending" };
      } else {
        query = {
          $or: [
      ...
      // GHI LOG: Xem danh sách yêu cầu
      logUserActivity?.({
        action: "friend.requests.view",
        req,
        res,
        userId: currentUserId,
        role: req.user.role,
        description: `Xem danh sách yêu cầu kết bạn (${type})`,
        payload: { type, count: friendRequests.length },
      });
  ...
  // [GET] /api/friends - Danh sách bạn bè của mình
  async getFriends(req, res) {
    try {
      const currentUserId = req.user.userId;
      const { page = 1, limit = 20 } = req.query;
      ...
      const friends = friendships.map((friendship) => {
        const friend =
          friendship.userA._id.toString() === currentUserId
            ? friendship.userB
            : friendship.userA;
        return {
          ...friend.toObject(),
      ...
      // GHI LOG: Xem danh sách bạn bè
      logUserActivity?.({
        action: "friend.list.view",
        req,
        res,
        userId: currentUserId,
        role: req.user.role,
        description: "Xem danh sách bạn bè của bản thân",
  ...
  // [GET] /api/friends/:userId - Danh sách bạn bè của người khác
  async getFriendsByUserId(req, res) {
    try {
      const currentUserId = req.user.userId;
      const { userId: targetUserId } = req.params;
      ...
      // GHI LOG: Xem danh sách bạn bè của người khác
      logUserActivity?.({
        action: "friend.list.view.other",
        req,
        res,
        userId: currentUserId,
        role: req.user.role,
        target: { type: "user", id: targetUserId },
        description: "Xem danh sách bạn bè của người khác",
        payload: { targetUserId, page, limit, total },
      });
  ...
  // [GET] /api/friends/status/:userId - Kiểm tra trạng thái
  async getFriendStatus(req, res) {
    try {
      const currentUserId = req.user.userId;
      const { userId: targetUserId } = req.params;
      ...
      if (currentUserId === targetUserId) {
        return res.json({
          success: true,
          data: { status: "self", isFriend: false, friendRequest: null },
        });
      }
      ...
      res.json({
        success: true,
        data: { status: "none", isFriend: false, friendRequest: null },
      });
    } catch (error) {
      console.error("Error getting friend status:", error);
      res.status(500).json({
  ...
  async removeFriend(req, res) {
    try {
      ...
      const userAIdStr = friendship.userA.toString();
      const userBIdStr = friendship.userB.toString();
      const currentUserIdStr = currentUserId.toString();

      if (userAIdStr !== currentUserIdStr && userBIdStr !== currentUserIdStr) {
        return res.status(403).json({
          success: false,
          message: "Bạn không có quyền xóa bạn bè này",
        });
      ...
      const otherUserId =
        userAIdStr === currentUserIdStr ? friendship.userB : friendship.userA;
      ...
      io.to(`user_${currentUserIdStr}`).emit("friend_status_changed", {
        userId: currentUserIdStr,
        otherUserId: otherUserIdStr,
        status: "none",
      });
      io.to(`user_${otherUserIdStr}`).emit("friend_status_changed", {
        userId: otherUserIdStr,
        otherUserId: currentUserIdStr,
        status: "none",
      });

      io.to(`user_${currentUserIdStr}`).emit("friend_count_changed", {
        userId: currentUserIdStr,
        otherUserId: otherUserIdStr,
        action: "unfriend",
        change: -1,
      ...
      logUserActivity?.({
        ...
        userId: currentUserId,
        role: req.user.role,
        target: { type: "user", id: otherUserIdStr },
